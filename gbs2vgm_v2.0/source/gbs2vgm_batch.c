...
static int convert_track(const char *gbs_filename, struct m3u_entry *entry, int track_num,
	...
	if (has_loop) {
		/* For looping tracks: render one complete loop iteration */
		loop_cycles = 0;  /* Loop starts at beginning */
		target_cycles = entry->duration_sec * GB_CLOCK;
	} else {
		/* For non-looping tracks: render full duration + fadeout */
		loop_cycles = 0;
	...
	/* For looping tracks, mark loop point at the beginning (after first register writes) */
	int loop_marked = 0;
	...
	while (total_cycles < target_cycles) {
		...
		/* Mark loop point at the beginning for looping tracks */
		/* Wait until we have some register writes before marking */
		if (has_loop && !loop_marked && total_cycles > GB_CLOCK / 10) {  /* After ~0.1 second */
			if (samples_since_last_write > 0) {
				vgm_write_wait(vgm, samples_since_last_write);
				samples_since_last_write = 0;
			}
			...
			loop_marked = 1;
			/* Note: Do NOT reset last_write_cycles here */
		}
	}
	...
	printf("  Done: %ld cycles processed", total_cycles);
	if (debug_mode) {
