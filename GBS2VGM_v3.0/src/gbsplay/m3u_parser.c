...
static void parse_entry_line(struct m3u_info *info, const char *line) {
	...
	int subsong;
	int loop_count = 1;
	char *last_comma, *second_last_comma, *third_last_comma, *fourth_last_comma, *fifth_last_comma;
	char line_copy[1024];
	...
	*last_comma = '\0';
	...
	if (old_format) {
		/* Old format: skip the empty field before loop_count */
		second_last_comma = strrchr(line_copy, ',');
		if (!second_last_comma) return;
		*second_last_comma = '\0';

		/* Now find the time field */
		...
		*third_last_comma = '\0';
		...
		/* No intro-loop or fade in old format */
		intro_loop_str[0] = '\0';
		fade_str[0] = '\0';
	} else {
		/* New format: parse fade, intro-loop, and time */
		/* Find second last comma (before fade) */
		second_last_comma = strrchr(line_copy, ',');
		if (!second_last_comma) return;

		/* Parse fade (optional) */
		strncpy(fade_str, second_last_comma + 1, sizeof(fade_str) - 1);
		fade_str[sizeof(fade_str) - 1] = '\0';
		*second_last_comma = '\0';

		/* Find third last comma (before intro-loop) */
		third_last_comma = strrchr(line_copy, ',');
		if (!third_last_comma) return;

		/* Parse intro-loop (optional) */
		strncpy(intro_loop_str, third_last_comma + 1, sizeof(intro_loop_str) - 1);
		intro_loop_str[sizeof(intro_loop_str) - 1] = '\0';
		*third_last_comma = '\0';

		/* Find fourth last comma (before time) */
		fourth_last_comma = strrchr(line_copy, ',');
		if (!fourth_last_comma) return;

		/* Parse time */
		strncpy(time_str, fourth_last_comma + 1, sizeof(time_str) - 1);
		time_str[sizeof(time_str) - 1] = '\0';
		*fourth_last_comma = '\0';
	}
